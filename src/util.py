...
# on macOS, run sysctl -n hw.ncpu and accordingly change num_cpus, for other machines
num_cpus = 12
        ...
        for split_elem in split_elems:
            if len(chars_to_split) == 1:
                if split_elem.find("_") == -1:
                    ...
                    short_split_elem = "_".join(
                        [us_elem[:2] for us_elem in split_elem.split("_")]
                    )
            else:
                short_split_elem = shorten_split_elem(
                    split_elem, chars_to_split[1:]
                )
