...
class CHACPolicy():
    @store_args
    def __init__(self, input_dims, T, rollout_batch_size, agent_params, env, 
        ...
        for i in range(self.n_levels):
            ...
            if i == self.n_levels - 1:
                ...
                for j in range(len(proj_end_goal)):
                    if (
                        np.absolute(self.goal_array[i][j] - proj_end_goal[j])
            ...
            # If not highest layer, compare to subgoal thresholds
                ...
                assert (
                    len(proj_subgoal)
                    == len(self.goal_array[i])
                    == len(env.sub_goal_thresholds)
                ), "Projected subgoal, actual subgoal, and subgoal thresholds \
                should have same dimensions"
                # Check whether layer i's goal was achieved by checking whether
                #  projected state is within the goal achievement threshold
                for j in range(len(proj_subgoal)):
                    if (
                        np.absolute(self.goal_array[i][j] - proj_subgoal[j])
                        > env.sub_goal_thresholds[j]
                    ):
                        goal_achieved = False
                        break
    ...
    def _create_networks(self, agent_params):
        ...
        self.layers = [
            Layer(i, self.env, agent_params) for i in range(self.n_levels)
        ]
    ...
    def learn(self, num_updates):
        ...
        counter = 0
        return [
            self.layers[i].learn(num_updates, counter)
    ...
    def train(self, env, episode_num, eval_data, num_updates):
        ...
        if self.verbose:
            print(
                "Next End Goal: ",
                env.wrapped_env.final_goal,
                env.wrapped_env.goal,
            )
        ...
        goal_status, eval_data, max_lay_achieved = self.layers[
            self.n_levels - 1
    ...
    def __getstate__(self):
        ...
        for layer in self.layers:
            ...
            if hasattr(layer, "state_predictor"):
                state["torch"][
                    "fw_model" + l
                ] = layer.state_predictor.cpu().state_dict()
                state[
                    "fw_model" + l + "err_list"
                ] = layer.state_predictor.err_list

            if hasattr(layer, "representer"):
                state["torch"][
                    "reg_model" + l
                ] = layer.representer.cpu().state_dict()
                state["torch"][
                    "reg_old_model" + l
                ] = layer.representer_old.cpu().state_dict()
            ...
            if hasattr(layer, "representer"):
                layer.representer.to(self.device)
                layer.representer_old.to(self.device)
    ...
    def __setstate__(self, state):
        ...
        state["env"] = prepare_env(
            env_name, agent_params["time_scales"], state["input_dims"]
        )
        self.__init__(**state)
        self.env.agent = self
        ...
        for layer in self.layers:
            ...
            if hasattr(layer, "state_predictor"):
                layer.state_predictor.load_state_dict(
                    state["torch"]["fw_model" + l]
                )
                layer.state_predictor.err_list = state[
                    "fw_model" + l + "err_list"
                ]
                layer.state_predictor.min_err = np.min(
                    state["fw_model" + l + "err_list"]
                )
                layer.state_predictor.max_err = np.max(
                    state["fw_model" + l + "err_list"]
                )
            if hasattr(layer, "representer"):
                layer.representer.load_state_dict(state["torch"]["representer" + l])
                layer.representer_old.load_state_dict(
                    state["torch"]["representer_old" + l]
                )
