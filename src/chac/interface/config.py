...
def cached_make_env(make_env):
    """
    Only creates a new environment from the provided function if one has not 
    yet already been created. This is useful here because we need to infer 
    certain properties of the env, e.g. its observation and action spaces, 
    without any intend of actually using it.
    """
...
def prepare_params(kwargs):
    """Assign the given parameters from the API to the variables"""
    chac_params = dict()
...
def configure_policy(dims, params):
    ...
    agent_params = {
        ...
        "subgoal_penalties": -1.0 * time_scales,
        "atomic_noise": [
            params["atomic_noise"] for i in range(input_dims["u"])
        ],
        "subgoal_noise": [
            params["subgoal_noise"] for i in range(len(env.sub_goal_thresholds))
        ],
...
def load_policy(restore_policy_file, params):
    """Load a policy if the training should be continued from the stored policy"""
...
def configure_dims(params):
    """Extract the right dimesnions to pass to the models"""
