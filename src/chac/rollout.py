...
import sys
import pickle
from src.chac.utils import store_args
from tqdm import tqdm
from collections import deque
from src.util import convert_episode_to_batch_major
...
class Rollout:
    ...
    def reset_rollout(self, i):
        """Resets the `i`-th rollout environment, re-samples a new goal, 
        and updates the `initial_o` and `g` arrays accordingly.
        """
    ...
    def generate_rollouts(self, return_states=False):
        ...
        `T` with the current policy acting on it accordingly.
        """
        self.reset_all_rollouts()
        ...
        for t in range(self.T):
            ...
            if self.policy_action_params:
                policy_output = self.policy.get_actions(
                    o, ag, self.g, **self.policy_action_params
                )
            else:
                policy_output = self.policy.get_actions(o, ag, self.g)

            if isinstance(policy_output, np.ndarray):
                # get the actions from the policy output since actions should 
        ...
        if other_histories:
            for history_index in range(len(other_histories[0])):
                ...
                self.custom_histories[history_index].append(
                    [x[history_index] for x in other_histories]
                )
    ...
    def seed(self, seed):
        """Seeds each environment with a distinct seed 
        derived from the passed in global seed."""
...
class RolloutWorker(Rollout):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.time_scales = np.array(
            [int(t) for t in kwargs["time_scales"].split(",")]
        )
        self.eval_data = {}
    ...
    def train_policy(self, n_train_rollouts, n_train_batches):
        ...
        for episode in tqdm(
            range(n_train_rollouts), file=sys.__stdout__, desc="Train Rollout"
        ):
            ro_start = time.time()
            success, self.eval_data, train_duration = self.policy.train(
                self.env, episode, self.eval_data, n_train_batches
            )
    ...
    def generate_rollouts(self, return_states=False):
        ...
        success, self.eval_data, _ = self.policy.train(
            self.env, self.n_episodes, self.eval_data, None
        )
        self.success_history.append(1.0 if success else 0.0)
        self.n_episodes += 1
        ...
        for i in range(self.policy.n_levels):
            ...
            if q_prefix in eval_data.keys():
                if len(eval_data[q_prefix]) > 0:
                    logs += [
                        (
                            "{}avg_q".format(layer_prefix),
                            np.mean(eval_data[q_prefix]),
                        )
                    ]
                else:
