...
class Layer():
    def __init__(self, level, env, agent_params):
        ...
        # prioritize sampling
        # self.k = 10 # based on regularization paper
        self.high_ratio = 0.5
        self.low_ratio = 0.3
        self.candidate_idxs = np.array([i for i in range(self.buffer_size//self.time_limit)])
        ...
        self.representer = RepresentationNetwork(env, 2, self.abs_range, env.subgoal_dim)
    ...
    def sample_prioritized_data(self, states_array):
        ...
        cur_candidate_idxs = self.candidate_idxs[:episode_num * (self.time_limit + 1)]
        ...
        # print(episode_idxs.shape)
        t_samples = selected_idx[0]

        high_states = states_array[t_samples:]
        # idx = t_samples + self.k if (t_samples + self.k) < states_array.shape[1] else t_samples
        # print("states_array", states_array.shape, episode_idxs.shape, t_samples.shape)
        high_states_next = states_array[t_samples:]

        states = high_states
        next_state = states_array[t_samples:]
        train_data = np.array([states, next_state, high_states, high_states_next])
    ...
    def get_reward(self, new_goal, hindsight_goal, goal_thresholds):
        ...
        achieved = np.all(np.absolute(new_goal - hindsight_goal) < goal_thresholds, axis=-1)
        return achieved - 1.0
    ...
    def train(self, agent, env, subgoal_test=False, episode_num=None, eval_data={}):
        ...
        while True:
            ...
            if self.level > 0:
                if self.reg and self.replay_buffer.size > self.time_limit and episode_num > 0:
                    states_array = self.replay_buffer._states.numpy()
                    train_data, select_idx, reg_states = self.sample_prioritized_data(states_array)
                    # If regualization is enabled
                    reg_feature_old = self.representer_old(torch.tensor(reg_states, dtype=torch.float32).to("cpu"))
                    reg_feature_new = self.representer(torch.tensor(reg_states, dtype=torch.float32).to("cpu"))

                    stable_loss = (reg_feature_new - reg_feature_old).pow(2).mean()

                    # prioritize sampling
                    # current_buffer = self.replay_buffer

                    # 
                    train_data = torch.tensor(train_data, dtype=torch.float32).to('cpu')
                    state, next_state = self.representer(train_data[0]),self.representer(train_data[1])
                    min_dist = torch.clamp((state - next_state).pow(2).mean(dim=1), min=0.)

                    high_state, high_next_state = self.representer(train_data[2]),self.representer(train_data[3])
                    max_dist = torch.clamp(1 - (high_state - high_next_state).pow(2).mean(dim=1), min=0.)
                    ...
                    self.representer.representation_optim.zero_grad()
                    representation_loss.backward()
                    self.representer.representation_optim.step()
                    subgoal = self.representer(current_state_tensor)[0].detach().numpy()
                    # if any value is nan choose the regularisaton
                    if np.isnan(subgoal).any():
                        print(f"The representer network produced Nans choose the regularised version")
