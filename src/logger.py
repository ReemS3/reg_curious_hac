...
class TensorBoardOutputFormat(KVWriter):
    ...
    def launchPanels(self):
        ...
        while True:
            tb.configure(
                argv=[None, "--logdir", self.data_read_dir, "--port", str(port)]
            )
            try:
                url = tb.launch()
                break
            ...
            if port > 7000:
                error(
                    "Could not find an open port for tensorboard. Tensorboard has to be launched manually."
                )
                return
        self.tb_port = port
    ...
    def writekvs(self, kvs):
        def summary_val(k, v):
            ...
            try:
                ...
                debug(
                    "Trying to store {} in tensorboard but it cannot be converted to float".format(
                        v
                    )
                )
    ...
    if format_strs is None:
        if rank == 0:
            format_strs = os.getenv(
                "OPENAI_LOG_FORMAT", "stdout,log,csv"
            ).split(",")
        else:
